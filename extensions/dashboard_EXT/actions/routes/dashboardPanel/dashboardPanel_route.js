...
module.exports = {
    ...
    init: async (DBM, Dashboard) => {
        Dashboard.app.post("/api/:serverID/execute/:command", async (req, res) => {
            ...
            if (!command.dashboardMod) {
                console.warn(`[DashboardPanel] Attempt to execute admin-only mod "${commandName}" from server dashboard by user ${req.user?.id || 'unknown'}`);
            ...
            try {
                ...
                if (!commandModule?.run || typeof commandModule.run !== "function") {
                    ...
                    const result = await Promise.resolve(
                        commandModule.run(DBM, req, res, Dashboard, guild)
                    );
    ...
    run: (DBM, req, res, Dashboard) => {
        ...
        if (!server) {
            res.redirect(`https://discord.com/oauth2/authorize?client_id=${DBM.Bot.bot.user.id}&scope=bot&permissions=2146958591&guild_id=${req.params.serverID}`);
            return {
                skipRender: true
            };
        } else {
            ...
            const mainPrefix = (DBM?.Files?.data?.settings?.tag && DBM.Files.data.settings.tag.trim().length
                ...
                if (giveawayUtils && typeof giveawayUtils.isGiveawaySystemAvailable === 'function') {
                    ...
                    if (giveawaySystemAvailable) {
                        try {
                            ...
                            giveawayData.giveaways.active = allGiveaways.filter(g => 
                                g && !g.ended && (!g.endTime || g.endTime > now) && g.status !== 'ended'
                            );
                            giveawayData.giveaways.scheduled = allGiveaways.filter(g => 
                                g && (g.status === 'scheduled' || (g.scheduledStart && g.scheduledStart > now))
                            );
                            giveawayData.giveaways.ended = allGiveaways.filter(g => 
                                g && (g.ended || g.status === 'ended' || (g.endTime && g.endTime <= now))
                            ...
                            if (req.user && req.user.guilds && Array.isArray(req.user.guilds)) {
                                giveawayData.userServers = req.user.guilds.filter(g => 
