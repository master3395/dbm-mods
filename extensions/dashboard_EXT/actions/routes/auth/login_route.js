const AuthenticationBase = require('../../base/authentication-base');
...
module.exports = {
    ...
    handleLogin: async function(req, res, username, password, rememberMe, cache) {
        try {
            ...
            const sessionToken = await this.databaseAuth.createSession(
                user.id, 
                clientIP, 
                userAgent
            );
    ...
    handleLogout: async function(req, res, cache) {
        try {
    ...
    handleCheckSession: async function(req, res, cache) {
        try {
            const sessionToken = req.cookies.session_token;
