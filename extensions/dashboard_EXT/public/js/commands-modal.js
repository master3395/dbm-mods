(function () {
    ...
    function updateGuildSelect() {
        ...
        if (!guildOptions.length) {
            ...
            modalFootnote.textContent = 'Log in with a Discord account that manages at least one shared server to enable execution.';
        } else {
    ...
    function populateCommandModal(command) {
        ...
        renderList(modalParameters, (command.parameters || []).map(param => `${param.name || 'Parameter'}${param.required ? ' (required)' : ''} – ${param.description || 'No description provided.'}`));

        modalFootnote.textContent = guildOptions.length ? 'Command executes using the News Targeted dashboard runner with your selected target.' : '';
        updateGuildSelect();
    ...
    function populateEventModal(eventData) {
        ...
        modalFootnote.textContent = 'Events cannot be executed manually, but their configuration is visible here for review.';
        modalForm.reset();
        modalExecuteButton.disabled = true;
