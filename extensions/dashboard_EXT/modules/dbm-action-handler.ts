...
export class DBMActionHandler {
    ...
    private static async handleGenerateRandomNumber(action: any, context: DBMActionContext): Promise<void> {
        const min = parseInt(action.min) || 1;
        const max = parseInt(action.max) || 100;
    ...
    private static async handleWait(action: any, context: DBMActionContext): Promise<void> {
        const time = parseInt(action.time) || 1000;
    ...
    private static getMemberFromAction(action: any, context: DBMActionContext): djs.GuildMember | djs.User | null {
        switch (action.member) {
            case "0": return context.author || null;
            case "1": return context.member || null;
            case "2": return context.member || null;
            case "3": return context.member || null;
            case "4": return context.member || null;
            case "5": return context.member || null;
            default: return context.member || context.author || null;
        }
    }
    ...
    private static checkCondition(value: any, comparison: string, expectedValue: any): boolean {
        switch (comparison) {
            case "1": return value == expectedValue;
            case "2": return value != expectedValue;
            case "3": return value > expectedValue;
            case "4": return value < expectedValue;
            case "5": return value >= expectedValue;
            case "6": return value <= expectedValue;
            case "7": return String(value).includes(String(expectedValue));
            case "8": return !String(value).includes(String(expectedValue));
            case "9": return String(value).startsWith(String(expectedValue));
            case "10": return String(value).endsWith(String(expectedValue));
    ...
    private static createActionRowComponents(buttons: any[]): djs.ActionRowBuilder<djs.ButtonBuilder>[] {
        ...
        for (let i = 0; i < buttons.length; i += 5) {
            ...
            buttonGroup.forEach((button: any) => {
                const buttonBuilder = new djs.ButtonBuilder()
