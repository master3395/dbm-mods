module.exports = {
  ...
  mod: async function (DBM) {
    DBM.require = function (packageName) {
      ...
      const nodeModulesPath = path.join(__dirname, 'dashboard_EXT', 'node_modules', packageName);
    ...
    Dashboard.checkActions = async function () {
      try {
        ...
        try {
          fetchImplementation = fetch; // Native fetch in Node.js 18+
        } catch {
          // Fallback for older Node.js versions
          fetchImplementation = DBM.require('node-fetch');
        }

        let modsFetched = await fetchImplementation(
          'https://api.github.com/repos/greatplainsmodding/DBM-Dashboard-Mods/git/trees/master',
        )
          .then((res) => res.json())
          .then((data) => data.tree)
          .catch((err) => {
            console.log(chalk.yellow('(DBM Dashboard) ~ Could not fetch remote mods: ' + err.message));
            return null;
          });
        ...
        for (let mod of modsFetched) {
          ...
          if (!require('fs').existsSync(filePath)) {
            ...
            let fetchedMod = await fetchImplementation(mod.url)
              .then((res) => res.json())
              .then((data) => data.tree);
            ...
            for (let file of fetchedMod) {
              ...
              if (!require('fs').existsSync(filePath)) {
                let modFile = await fetchImplementation(
                  'https://raw.githubusercontent.com/greatplainsmodding/DBM-Dashboard-Mods/master/' +
                    mod.path +
                    '/' +
                    file.path,
                ).then((res) => res.text());
                require('fs').writeFileSync(filePath, modFile, 'utf8');
        ...
        console.log(chalk.red('Dashboard mod check error: ' + error.message));
      }
    };
    ...
    Dashboard.loadRoutes = function () {
      require('fs')
        .readdirSync(Dashboard.Actions.routeLocation)
        .forEach((dir) => {
          ...
          if (routeData.isCustom) {
            ...
            routeData.scriptFiles.forEach((file) => {
              ...
              const webFile = require('path').join(
                __dirname,
                'dashboard_EXT',
                'actions',
                'routes',
    ...
    Dashboard.appSettings = function () {
      ...
      Dashboard.app.use(express.json());
