...
//---------------------------------------------------------------------
// Check Discord.JS version - Support both v13.14.0 and v14.26.2
//---------------------------------------------------------------------
...
const requiredVersion = isV14 ? "14.26.2" : "13.14.0";
			...
			channel.messages
				...
				.then((message) => {
					...
					if (packet.t === "MESSAGE_REACTION_ADD") {
						(client as NodeJS.EventEmitter).emit(
							"messageReactionAdd",
							reaction,
	...
	static mergeSubCommandIntoCommandData(names, data) {
		...
		if (names.length === 2) {
			...
			if (
				...
				(chatCommand.options as djs.ApplicationCommandOptionData[]).push(data);
			}
			...
			if (baseGroup === null) {
				...
				(chatCommand.options as djs.ApplicationCommandOptionData[]).push(baseGroup);
...
export class Actions {
	...
	static async checkConditions(guild: djs.Guild | null, member: djs.GuildMember | djs.APIGuildMember | null, user: djs.User, cmd: dbm.Command): Promise<boolean> {
		...
		switch (restriction) {
			...
			case 1: {
				if (isServer) {
					const guildMember = member as djs.GuildMember;
					if(Array.isArray(cmd.requiredPermissions)) {
						return this.checkPermissions(guildMember,
					...
					return (
						this.checkPermissionString(guildMember, cmd.permissions) &&
						this.checkPermissionString(guildMember, cmd.permissions2)
					);
				}
				return restriction === 0;
			}
			case 2:
				return isServer && guild!.ownerId === (member as djs.GuildMember).id;
			case 3:
				return !isServer;
			case 4:
...
export class Images {
	...
	static getImage (url: string): Promise<any> {
	...
	static createBuffer (image: any): Promise<Buffer> {
	...
	static drawImageOnImage (img1: any, img2: any, x: number, y: number) {
