...
WrexMODS.runPostJson = function (url, json, returnJson = true, callback){
	...
	var axios = this.require('axios');
	axios.post(url, json, { headers: { 'Content-Type': 'application/json' } })
		.then(function (res) {
			var statusCode = res && res.status ? res.status : 200;
		...
		.catch(function (err) {
			var statusCode = err.response && err.response.status ? err.response.status : 500;
			var data = err.response && err.response.data !== undefined ? err.response.data : null;
			if (callback && typeof callback === "function") {
...
WrexMODS.executeDiscordJSON = function(type, urlPath, json ,DBM, cache, callback){
	return new Promise((resolve, reject) => {
		var axios = this.require('axios');
		var url = "https://discordapp.com/api/v6/" + urlPath;
		var config = {
...
WrexMODS.runPublicRequest = function (url, returnJson = false, callback, token, user, pass){
    ...
    /// <param name="callback" type="Function">The callback function, args: error, statusCode, data</param>  
    var axios = this.require("axios");
	var config = { url: url, headers: { 'User-Agent': 'Other' }, responseType: returnJson ? 'json' : undefined };
	if (token) config.headers['Authorization'] = 'Bearer ' + token;
	if (user != null && pass != null) config.auth = { username: user, password: pass };
...
WrexMODS.runBearerTokenRequest = function (url, returnJson = false, bearerToken, callback){
	...
	/// <param name="callback" type="Function">The callback function, args: error, statusCode, data</param>  
    var axios = this.require("axios");
	var config = { url: url, headers: { 'User-Agent': 'Other', 'Authorization': 'Bearer ' + bearerToken }, responseType: returnJson ? 'json' : undefined };
	axios.get(url, config)
		.then(function (res) {
			var statusCode = res && res.status ? res.status : 200;
			var data = res && res.data !== undefined ? res.data : res;
...
WrexMODS.runBasicAuthRequest = function (url, returnJson = false, username, password, callback){
	...
	/// <param name="callback" type="Function">The callback function, args: error, statusCode, data</param>  
    var axios = this.require("axios");
	var config = { url: url, headers: { 'User-Agent': 'Other' }, auth: { username: username, password: password }, responseType: returnJson ? 'json' : undefined };
	axios.get(url, config)
		.then(function (res) {
			var statusCode = res && res.status ? res.status : 200;
			var data = res && res.data !== undefined ? res.data : res;
