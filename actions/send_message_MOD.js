...
<div id="xinxyla3">
  ...
  async action(cache) {
    ...
    if (Array.isArray(data.components) && data.components.length > 0) {
      if (storagewebhook > 0) {
        this.displayError(data, cache, '[Send Message MOD] Components V2 is not supported in webhook mode.');
      ...
      try {
        ...
        this.displayError(data, cache, `[Send Message MOD] Failed Components V2 delegation: ${err?.message ?? err}`);
        return this.callNextAction(cache);
      }
    }
    ...
    if (data.attachments?.length > 0) {
      ...
      for (let i = 0; i < data.attachments.length; i++) {
        if (String(data.attachments[i].type) === '1') {
        ...
        if (String(data.attachments[i].type) === '2') {
        ...
        if (String(data.attachments[i].type) === '0' || data.attachments[i].type === undefined) {
          const attachment = data.attachments[i];
